...
class BaseDatasetWrapper(Dataset):
    ...
    def __getitem__(self, index):
        sample, target, index = self.org_dataset.__getitem__(index)
...
class PolicyDatasetC10(BaseDatasetWrapper):
    ...
    def __getitem__(self, index):
        sample, target, index = super(PolicyDatasetC10, self).__getitem__(index)
...
class PolicyDatasetC100(BaseDatasetWrapper):
    def __init__(self, org_dataset, multiple, p=0.3):
    ...
    def __getitem__(self, index):
        ...
        if self.multiple == "4X":
            samples = []
            targets = []
            # Generate two copies of the original and two augmented samples
            for _ in range(2):

                sample, target, index = super(PolicyDatasetC100, self).__getitem__(index)
                if isinstance(target, torch.Tensor) and target.ndim == 2 and target.shape[-1] != 1:
                    target = target.argmax(1)
                elif not isinstance(target, torch.Tensor):
                    target = torch.LongTensor([target])

                transformed_sample = self.transform(sample).detach()  # Original samples
                samples.append(transformed_sample)
                targets.append(target)

                new_sample = sample  # Augmented samples
                for i in range(self.policies_len):
                    new_sample, _ = self.policies[i](new_sample)
                transformed_new_sample = self.transform(new_sample).detach()
                samples.append(transformed_new_sample)
                targets.append(target)

            sample = torch.stack(samples)  # Stack all samples
            target = torch.stack(targets)  # Stack all targets
        ...
        return sample, target, index  # This project does not use indexes, so it is not processed
...
class RADatasetC100(BaseDatasetWrapper):
    ...
    def __getitem__(self, index):
        sample, target, index = super(RADatasetC100, self).__getitem__(index)
        new_sample = sample
...
class AADatasetC100(BaseDatasetWrapper):
    ...
    def __getitem__(self, index):
        sample, target, index = super(AADatasetC100, self).__getitem__(index)
        new_sample = sample
        new_sample = self.RandAugment(new_sample)
        new_sample = self.transform(new_sample).detach()
        sample = self.transform(sample).detach()
...
class PolicyDatasetImageNet(BaseDatasetWrapper):
    def __init__(self, org_dataset, multiple, p=0.2):
    ...
    def __getitem__(self, index):
        ...
        # Generate two copies of the original and two augmented samples
        for _ in range(2):
            sample, target, index = super(PolicyDatasetImageNet, self).__getitem__(index)
            if isinstance(target, torch.Tensor) and target.ndim == 2 and target.shape[-1] != 1:
                target = target.argmax(1)
            elif not isinstance(target, torch.Tensor):
                target = torch.LongTensor([target])
